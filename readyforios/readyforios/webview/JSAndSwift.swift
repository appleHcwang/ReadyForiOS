...
class JSAndSwift: UIViewController {
    ...
    private func initUI(){
        ...
        config.userContentController.add(self, name: "WHCJSObject")
...
//        let url = URL.init(fileURLWithPath:Bundle.main.path(forResource: "index", ofType: "html", inDirectory: "vue")!)
        let url = URL.init(string: "http://10.8.6.60:2021/")
    ...
    override func viewWillDisappear(_ animated: Bool) {
        ...
        self.webView?.configuration.userContentController.removeScriptMessageHandler(forName: "WHCJSObject")
    }
...
//        self.present(alert, animated: true, completion: nil)
        if message.name == "WHCJSObject" {
            self.webView?.evaluateJavaScript("shareResult('nnnnn')", completionHandler: nil)
        }
    }
}
